...
export const PokemonCard = ({ pokemon }: { pokemon: generalLink }) => {
  ...
  useEffect(() => {
    async function fetchPokemon() {
      ...
      const size = pokemonData.height > 15 ? 200 : pokemonData.height >= 8 ? 150 : 110;

      const filteredMoves = pokemonData.moves.filter(
      ...
      const randomMoves =
        filteredMoves.length > 1
          ? [filteredMoves[index == 0 ? 0 : index - 1], filteredMoves[index == 0 ? 1 : index]]
          : [filteredMoves[0]];
  ...
  return (
    ...
    >
      {pokemonSpecies && pokemonSpecies.varieties.length > 1 ? (
        ...
        <PokemonImage pokemonData={pokemonData} pokemonSize={pokemonSize} />
      )}
      <section className={styles.bottom}>
        <div className={styles.types}>
          {pokemonCardData?.types.map((type, i) => <TypeBadge name={type} key={i} />)}
        </div>
